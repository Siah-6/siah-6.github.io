<div class="container">
    <h2 class="fw-bold mb-4 text-center" style="letter-spacing:1px;">All Melee Skins</h2>
    <div class="row row-cols-1 row-cols-md-2 g-4">
        <?php
        $melees = [
            "Knife", "Axe", "Dagger", "Claw", "Glitchpop", "Blade", "Reaver", "Hammer"
        ];
        foreach ($melees as $melee):
            $img = strtolower(str_replace([' ', '/'], ['-', ''], $melee));
            $imgPath = "assests/images/melee/{$img}.jpg";
            if (!file_exists($imgPath)) {
                $imgPath = "assests/images/melee/{$img}.png";
            }
        ?>
            <div class="col">
                    ...
                    <div class="card-body d-flex flex-column justify-content-center align-items-center p-0 w-100">
                        <div class="weapon-image-container" style="height:260px;">
                            <?php if (file_exists($imgPath)): ?>
                                <img src="<?= $imgPath ?>" alt="<?= $melee ?>" class="weapon-image" style="max-height:220px;max-width:98%;box-shadow:0 8px 32px #ff465544;border-radius:18px;transition:transform 0.3s,box-shadow 0.3s;">
                            <?php else: ?>
                        ...
                        <div class="weapon-info">
                            <span class="weapon-name"><?= $melee ?></span>
                        </div>
                    </div>
                </div>
            </div>
